...
export function SubmitForm({ config, onSubmitted, disabled }: SubmitFormProps) {
  ...
  return (
    <div className="rounded-lg border border-surface-border bg-surface-card p-4">
      <h2 className="mb-3 text-sm font-medium text-zinc-200">Submit a claim</h2>
      <form onSubmit={handleSubmit} className="space-y-3">
        <textarea
          ...
          className="w-full resize-none rounded-md border border-surface-border bg-black/30 px-3 py-2 font-mono text-sm text-zinc-100 placeholder:text-zinc-600 focus:border-zinc-500 focus:outline-none disabled:opacity-50"
        />
        <select
          value={source}
          onChange={(e) => setSource(e.target.value)}
          disabled={loading || disabled}
          className="w-full rounded-md border border-surface-border bg-black/30 px-3 py-2 text-sm text-zinc-200 focus:border-zinc-500 focus:outline-none disabled:opacity-50"
        >
        ...
        <button
          ...
          className="flex w-full items-center justify-center gap-2 rounded-md bg-zinc-100 px-4 py-2 text-sm font-medium text-black transition hover:bg-white disabled:cursor-not-allowed disabled:opacity-40"
        >
          {loading && (
